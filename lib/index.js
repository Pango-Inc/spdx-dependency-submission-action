...
function getManifestFromSpdxFile(document, fileName) {
    ...
    document.packages?.forEach(pkg => {
        ...
        try {
            ...
            // A package is direct when only the document's root(s) depend on it.
            // Detect the root from documentDescribes / DESCRIBES (with the
            // SPDXRef-RootPackage convention as a fallback) instead of assuming a
            // single hardcoded root SPDXID.
            ...
            // A non-root depender means this package is transitive. Consider BOTH
            // relationship directions: DEPENDS_ON (depender = spdxElementId) and its
            // spec-equivalent inverse DEPENDENCY_OF (depender = relatedSpdxElement),
            // since converters such as syft emit DEPENDENCY_OF.
